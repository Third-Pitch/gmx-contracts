...
import "./interfaces/IEddxIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IEddxMigrator.sol";

contract EddxMigrator is ReentrancyGuard, IEddxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "EddxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "EddxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "EddxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "EddxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "EddxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "EddxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "EddxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "EddxMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        eddxPrice = _eddxPrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "EddxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "EddxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "EddxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "EddxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(eddxPrice);
        require(mintAmount > 0, "EddxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "EddxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "EddxMigrator: invalid tokenA");
            require(tokenB != address(0), "EddxMigrator: invalid tokenB");
        ...
        IEddxIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "EddxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "EddxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "EddxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "EddxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("EddxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "EddxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "EddxMigrator: invalid _action");
