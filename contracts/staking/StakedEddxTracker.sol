...
contract StakedEddxTracker is IERC20, ReentrancyGuard, IRewardTracker, Governable {
    ...
    // add coldTime 60 days
    uint256 public coldTime = 60 * 60 * 24 * 60;
    mapping (address => uint256) public coldTimes;
    ...
    function setColdTime(uint256 _coldTime) external onlyGov {
        coldTime = _coldTime;
    }
    ...
    function _stake(address _fundingAccount, address _account, address _depositToken, uint256 _amount) private {
        if(coldTimes[msg.sender] < uint256(1)){
            coldTimes[msg.sender] = block.timestamp + coldTime;
        }
    ...
    function _unstake(address _account, address _depositToken, uint256 _amount, address _receiver) private {
        require(_amount > 0, "RewardTracker: invalid _amount");
        require(isDepositToken[_depositToken], "RewardTracker: invalid _depositToken");
        require(coldTimes[msg.sender] <= block.timestamp, "unStake later");
