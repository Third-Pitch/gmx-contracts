...
contract VesterV2 is IVester, IERC20, ReentrancyGuard, Governable {
    ...
    uint256 public VEST_INTERVAL = 30 days;
    ...
    function deposit(uint256 _amount, uint256 _months) external nonReentrant {
        _deposit(msg.sender, _amount, _months);
    }

    function depositForAccount(address _account, uint256 _amount, uint256 _months) external nonReentrant {
        _validateHandler();
        _deposit(_account, _amount, _months);
    }
    ...
    function _deposit(address _account, uint256 _amount, uint256 _months) private {
        require(_amount > 0, "Vester: invalid _amount");
        require(uint256(0) <= _months && _months <= uint256(12), "Vester: invalid months");

        _updateVesting(_account);

        vestingDurations[_account] = _months.mul(VEST_INTERVAL);
    ...
    function _updateVesting(address _account) private {
        (uint256 amount, uint256 balance) = _getNextClaimableAmount(_account);
        ...
        IMintable(esToken).burn(address(this), balance);
    }

    function _getNextClaimableAmount(address _account) private view returns (uint256 claimableAmount, uint256 balance) {
       ...
       balance = balances[_account];
        if (balance > 0) {
            ...
            if(timeDiff >= vestingDuration) {
                timeDiff = vestingDuration;
            }else{
                balance = balance.mul(timeDiff).div(vestingDuration);
            }
            // vestedAmount.mul(timeDiff).mul(vestingDuration.div(VEST_INTERVAL)).div(vestingDuration)
            claimableAmount = vestingDuration == uint256(0) ? vestedAmount : vestedAmount.mul(timeDiff).div(VEST_INTERVAL);
        }
    }
