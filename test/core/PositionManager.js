  ...
  beforeEach(async () => {
    ...
    deployTimelock = async () => {
      return await deployContract("Timelock", [
        ...
        ethers.constants.AddressZero, // _elpManager
...
describe("PositionManager next short data calculations", function () {
  ...
  async function debugState(label = "") {
    ...
    const aum = await elpManager.getAum(true)
  ...
  it("PositionManager and ElpManager init with shortsTracker", async () => {
    const [
      positionManagerShortTracker,
      elpManagerShortTracker,
      avgeragePrice,
      size
    ] = await Promise.all([
      positionManager.shortsTracker(),
      elpManager.shortsTracker(),
    ...
    expect(elpManagerShortTracker, 'elpManager shortsTracker').eq(shortsTracker.address)
  ...
  it("updates global short average prices on position decreases", async () => {
    await shortsTracker.setIsGlobalShortDataReady(true)
    await elpManager.setShortsTrackerAveragePriceWeight(10000)
    expect(await elpManager.shortsTrackerAveragePriceWeight()).to.be.equal(10000)
    ...
    let aumBefore = await elpManager.getAum(true)
    ...
    let aumAfter = await elpManager.getAum(true)
    ...
    aumBefore = await elpManager.getAum(true)
    ...
    aumAfter = await elpManager.getAum(true)
